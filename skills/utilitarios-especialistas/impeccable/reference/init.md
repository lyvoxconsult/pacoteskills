...
- **Improve what's there**: name the specific surface. `$impeccable critique <page>` for a scored UX review; `$impeccable audit <area>` for a11y / perf / responsive checks; `$impeccable polish <component>` for a pre-ship pass. When the crawl flagged a specific weakness, point the matching command at it: thin hierarchy or spacing â†’ `layout`, flat or gray palette â†’ `colorize`, missing error / empty states â†’ `harden` or `onboard`, dull or unclear copy â†’ `clarify`.
