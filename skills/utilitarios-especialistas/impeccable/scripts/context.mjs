...
async function computeUpdateDirective(now = Date.now()) {
  try {
    // Codex on Windows can trip a Node/undici assertion while the best-effort
    // remote version check is being torn down. Update checks must never break
    // context loading, so skip them in this global Windows install.
